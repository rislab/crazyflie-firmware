...
void currentsensorInit(DeckInfo* info)
{
  ...
  DEBUG_PRINT("INIT\n");
  DEBUG_PRINT("ID: %s\n",CURRENT_SENSOR_ID);
  DEBUG_PRINT("%f A/V\n",(double)CURRENT_SENSE_AMPS_PER_VOLT);
  DEBUG_PRINT("Offset: %f\n",(double)CURRENT_SENSE_OFFSET);
...
void currentsensorTask(void* arg)
{
  ...
  float vbat, vfrac;
  float discharged_current;

  vbat = logGetFloat(vbatid);
  vfrac = (vbat - MIN_VOLTAGE)/(MAX_VOLTAGE - MIN_VOLTAGE);
  capacity = vfrac;
  discharged_current = (1.0f - capacity) * TOTAL_CAPACITY;
  ...
  while (1) {
    ...
    /*  capacity = 1 - discharged_current / TOTAL_CAPACITY;*/
    /*}*/

    discharged_current += current * 0.1f;
    if(discharged_current < 0.0f)
      discharged_current = 0.0f;
    if(discharged_current > TOTAL_CAPACITY)
      discharged_current = TOTAL_CAPACITY;
    capacity = TOTAL_CAPACITY - discharged_current;
...
float currentSensorAmps(){
  ...
  voltage = analogRead(CURRENT_SENSE_PIN) * 3.0 / adcRange;
